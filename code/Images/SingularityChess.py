...
def printUsage():
	print "Usage: python SingularityChess.py whiteAgent blackAgent [simulations]"
	print "whiteAgent and blackAgent can be one of [HumanAgent, RandomAgent, MinimaxAgent, MonteCarloAgent]"
	print "simulations, the optional parameter, determines whether or not simulation mode is used."
	print "simulation mode disables graphics and plays out the given number of simulations internally if set;"
	print "it cannot be used with a HumanAgent"
	sys.exit(1)

def getAgentFromArg(arg, color, simulation, tkThing=None):
	if arg == "HumanAgent":
		if simulation:
			print "Error: Cannot use HumanAgent in simulation mode"
			printUsage()
		return HumanAgent(color, tkThing)
	elif arg == "RandomAgent":
...
def getAndDrawNextMove():
	...
	root.after(10, getAndDrawNextMove)
